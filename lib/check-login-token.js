...
Since checking the login token requires access to the database through the RadditAPI,
this function receives the raddit API object and then returns a middleware.

The middleware will check request.cookies for a SESSION token. if it exists, the middleware will
check through RadditAPI if the token represents a valid user. If it does, then the user object will
...
module.exports = function(myRaddit) {
    return function checkLoginToken(request, response, next) {
        ...
        if (request.cookies.SESSION) {
            myRaddit.getUserFromSession(request.cookies.SESSION)
            .then(function(user) {
